//! One-off migration: seal legacy plaintext / client-opaque message bodies in MongoDB.
//!
//! Run **after** deploying the backend that serves opaque `content` via API.
//! Safe to run while the server is online (updates are conditional on unchanged `content`).
//!
//! Usage:
//!   cargo run --bin migrate-message-content-seal -- --dry-run
//!   cargo run --bin migrate-message-content-seal -- --apply
//!   cargo run --bin migrate-message-content-seal -- --apply --limit 100
//!
//! Requires the same `.env` as the main server (`DATABASE_URL`, `JWT_KEY` or `FIELD_ENCRYPTION_KEY`).

use klovy_chat_server::utils::database_url::database_url;
use klovy_chat_server::utils::db;
use klovy_chat_server::utils::messages::seal_legacy_content::{
...
fn print_help() {
    eprintln!(
        "migrate-message-content-seal — seal legacy message content in MongoDB\n\
         \n\
         Options:\n\
